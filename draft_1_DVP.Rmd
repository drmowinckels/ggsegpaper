...
Most users will use ggseg to display through a color scale some descriptive or inferential statistics such as mean cortical thickness or thickness – cognition relationship across the different brain regions.
...
Yet the data in ‘ggseg’ is in a long format - that is each region has its own row - thus data of interest needs to be converted into this format. 
...
Yet brains are intrinsically 3D and it can be challenging to recognize the location of a region. This problem exacerbates in atlases that represent subcortical features – after all, cortical surfaces are 2D – such as grey matter structures or white matter tracts. 
...
The 'LCBC' surface consists on a semi-inflated white matter surface based on the _fsaverage_ template subject. See *SCRIPT* for reproducibility.
All `[...]_3d` atlases have a built in `colour` column for default colour plotting of the regions, based on the colorlut used in the neuroimaging sofware.
...
The cortical atlas data is stored in nested tibbles. Each atlas have datasets for three different surfaces (see Figure X) and the two hemispheres.
The 'ggseg3d' column includes all necessary information to `ggseg3d()` to create a mesh-plot. 
The additional `[..._3d]` atlases in the ggsegExtra contain identical structures except for the subcortical atlases in which surface and hemisphere do not apply. 
...
First, one needs to grab the data for a specific surface (and hemisphere, if desired) and then `unnest()` it. 
...
Each list corresponds to a regio and contains 6 vectors required to create the mesh of the tri-surface plot. 

Note als the `label`, `annot` and `area` columns which provide matching values for your own data. 
Similarly to ggseg function, the label column should match the region names used in the original neuroimaging software while area and annot provide alternative/ secundary names. 
It is thus important to match your identifiers for region with those used in the atlas. 
...
Subcortical atlases include cortical surfaces and other landmark structures to visualization purposes only. 
One can control the opacity of the these `NA` structures, to improve visualization. 
Additionaly, one can add the `glassbrain` option, which provides a frame of reference for the subcortical structures.
Glassbrain can be controlled with thre different arguments: opacity (0-1), hemisphere (left, right), and colour.
...
ggsed3d() are based on plotly and thus can be additional plotly functionalities will can be used to modify and improve the 3D atlas representations. 
...
Finally, we recommend [orca](https://github.com/plotly/orca#installation) command line to save ggseg3d atlas snapshots.  
