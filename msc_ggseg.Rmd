...
pkglist <- c("ggseg", "ggseg3d",  
             basename(ggsegExtra::ggseg_atlas_repos()$repo))
dt <- lapply(pkglist, get_data_list) %>% 
  bind_rows() %>% 
  ...
  mutate(Citation = case_when(
    grepl("dk", Item) ~ "\\citet{dk}",
    grepl("yeo", Item) ~ "\\citet{yeo2011}",
    grepl("aseg|hcpa", Item) ~ "\\citet{aseg}",
...
A summary of all available atlases compatible with the _ggseg_-packages as of `r format(Sys.Date(), "%B %Y")` can be found in Table \@ref(tab:table1).


```{r 'table1', echo=F, warning=FALSE, message=FALSE}
dt %>%
  kable(format="latex", escape = FALSE, booktabs = TRUE,
        caption = "Table of currently available atlases in ggseg and ggseg3d, and listed in the ggsegExtra R-package. Polygon and Mesh refer to 2D and 3D brain atlas representations, respectively. Cell contents in the Mesh and Polygon columns indicate the source package containing the atlas data-set.") %>%
...
```{r "figure7-show", message=FALSE, echo=TRUE, results = 'hide',  out.width='30%',fig.show='hold',fig.ncol = 3}
...
```{r "figure7", message=FALSE, echo=F, eval=TRUE, out.width='30%',fig.show='hold',fig.ncol = 3, fig.cap='(ref:figure7-cap)'}
orca(fig7l, file = "msc_ggseg_files/figure-latex/figure7l.png", format = "png")
orca(fig7m, file = "msc_ggseg_files/figure-latex/figure7m.png", format = "png")
orca(fig7r, file = "msc_ggseg_files/figure-latex/figure7r.png", format = "png")

include_graphics(c("msc_ggseg_files/figure-latex/figure7l.png",
                   "msc_ggseg_files/figure-latex/figure7m.png",
                   "msc_ggseg_files/figure-latex/figure7r.png"))
```
...
```{r out.width='80%', linewidth=100, size='small', eval=TRUE}
...
```{r 'figure8-show', include=FALSE, out.width='40%', results='hide'}
# Figure 8 left
...
```{r 'figure8', out.width='30%', echo=FALSE, fig.cap="(ref:figure8-cap)", fig.show='hold', fig.ncol = 3}
orca(fig8l, file = "msc_ggseg_files/figure-latex/figure8l.png", format = "png")
orca(fig8m, file = "msc_ggseg_files/figure-latex/figure8m.png", format = "png")
orca(fig8r, file = "msc_ggseg_files/figure-latex/figure8r.png", format = "png")

include_graphics(c(
  "msc_ggseg_files/figure-latex/figure8l.png",
  "msc_ggseg_files/figure-latex/figure8m.png",
  "msc_ggseg_files/figure-latex/figure8r.png"))
...
include_plotly(fig9, "msc_ggseg_files/figure-latex/figure9.png")
```
...
invisible(ggsave(filename = "msc_ggseg_files/figure-latex/figure10ab.png", 
                 plot = ggsegs_p, device = "png"))
orca(desterieux_p, file = "msc_ggseg_files/figure-latex/figure10c.png", format = "png")
orca(icbm_p, file = "msc_ggseg_files/figure-latex/figure10d.png", format = "png")

include_graphics(c(
  "msc_ggseg_files/figure-latex/figure10ab.png",
  "msc_ggseg_files/figure-latex/figure10c.png",
  "msc_ggseg_files/figure-latex/figure10d.png"
))
```
...
This is exemplified in the [online supplementary information](https://athanasiamo.shinyapps.io/Virtual_histology_2019/) of Vidal-Piñeiro et al. (-@vidal_2019).

The functions in the _ggseg_-packages provide users with the possibility of adapting the plots to their wishes, and also makes it possible to create and contribute to the atlas repository in _ggsegExtra_.
...
3. Complimentary features, like color scales, and functions to convert compatible data frames to atlas-classes, read in likely data sources (like Freesurfer analysis output), and help create new atlases for use with _ggseg_ and _ggseg3d_.  
...
The ggseg-packages are based on _ggplot2_ and _plotly_, which are only two of several visualisation tools in R, in addition to base-R plotting. 
While the functionality presented here could also be integrated into packages such as [lattice](https://www.rdocumentation.org/packages/lattice/versions/0.20-40), both _ggplot2_ and _plotly_ have become established tools in the R-community and have extensive online documentation.
...
In terms of planned _ggseg_ improvements, we are working on making a proper `geom` or `stat`.
While `ggseg()` works quite nicely as a `ggplot` wrapper, it introduces some unexpected work for the user, like having to group data _before_ faceting. 
A `geom` or `stat` would benefit the user as it would behave more like other _ggplot2_ `geoms` and there would be less new functionalities to learn.

Concerning _ggseg3d_, we also hope to make it possible to colour each individual mesh triangle, and as such provide the users with more flexibility in the plotting. 
...
Loading the entire brain mesh at once, and then coloring mesh triangles would significiantly improve the speed of `ggseg3d()`. 
